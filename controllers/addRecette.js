...
const verifAddRecetteController = async (req, res) => {
...
const sqlInsRecettes = "INSERT INTO Recettes (titre, categories_ID, description, image) VALUES (?,?,?,?)"
...
const addRecetteController = (req, res) => {
    ...
    // Etape 1.1    
         ...
         */
        const addRecette = await asyncQuery(sqlInsRecettes,domReqRecettes)
...
const insertRecetteIngredient = async (data, idRecette) => {
    ...
    for (let index = 0; index <= data.length; index++) {
        ...
        if (index === data.length) {
            ...
            const insRecIngr = await asyncQuery(sqlInsRecIngr,[result])
            return insRecIngr
        }
        ...
        // Etape 7
const removeDoublon = async (type) => {
    
    const data = {
        unite:{
            sqlGetDoublon:'SELECT COUNT(nom) AS nb, nom FROM Unites GROUP BY nom HAVING COUNT(nom) > 1',
            sqlGetIdToDelete:'SELECT id FROM Unites WHERE nom = ? ORDER BY id DESC LIMIT 1',
            sqlUpdateID: `UPDATE Recettes_Ingredients SET unites_ID = (SELECT id FROM Unites WHERE nom = ? ORDER BY id ASC LIMIT 1) WHERE unites_ID = (SELECT id FROM Unites WHERE nom = ? ORDER BY id DESC LIMIT 1)`,
            sqlDeleteDoublon: "DELETE FROM `Unites` WHERE id = ?"
        },
        quantite:{
            sqlGetDoublon:'SELECT COUNT(nom) AS nb, nom FROM Quantites GROUP BY nom HAVING COUNT(nom) > 1',
            sqlGetIdToDelete:'SELECT id FROM Quantites WHERE nom = ? ORDER BY id DESC LIMIT 1',
            sqlUpdateID: `UPDATE Recettes_Ingredients SET quantites_ID = (SELECT id FROM Quantites WHERE nom = ? ORDER BY id ASC LIMIT 1) WHERE unites_ID = (SELECT id FROM Quantites WHERE nom = ? ORDER BY id DESC LIMIT 1)`,
            sqlDeleteDoublon: "DELETE FROM Quantites WHERE id = ?"
        },
        ingredient:{
            sqlGetDoublon:'SELECT COUNT(nom) AS nb, nom FROM Ingredients GROUP BY nom HAVING COUNT(nom) > 1',
            sqlGetIdToDelete:'SELECT id FROM Ingredients WHERE nom = ? ORDER BY id DESC LIMIT 1',
            sqlUpdateID: `UPDATE Recettes_Ingredients SET ingredients_ID = (SELECT id FROM Ingredients WHERE nom = ? ORDER BY id ASC LIMIT 1) WHERE unites_ID = (SELECT id FROM Ingredients WHERE nom = ? ORDER BY id DESC LIMIT 1)`,
            sqlDeleteDoublon: "DELETE FROM Ingredients WHERE id = ?"
        },
    }
...
export {addRecetteController, verifAddRecetteController, removeDoublon};